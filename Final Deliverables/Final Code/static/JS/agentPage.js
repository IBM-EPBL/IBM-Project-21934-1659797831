...
try {
    ...
    if(id.id == 0){
        location.href = `${location.origin}/tempLogin`
    }else{
...
document.querySelectorAll('.ticket-box').forEach(val => {
    val.addEventListener('click', async () => {
        ...
        else if(String(content.session_link) != "") {
            document.querySelector('.link-sess').setAttribute('disabled', '')
            document.querySelector('button[role=Status]').setAttribute('disabled', '')
        }
