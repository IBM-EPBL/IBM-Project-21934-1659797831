            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response[0].Success == "Ok") {
                        selected_ticket.childNodes[3].childNodes[1].childNodes[3].childNodes[1].setAttribute('class','ticket-status green')
                        selected_ticket.childNodes[3].childNodes[1].childNodes[3].childNodes[1].innerText = 'Opened'
...
document.querySelectorAll('.ticket-box').forEach(val => {
    val.addEventListener('click', async () => {
        ...
        setTimeout(() => {
            ...
            document.querySelectorAll('.loading-spin')[2].classList.add('disabled')
