...
async function getCompanyList (){ 
    await fetch('https://jp-tok.functions.appdomain.cloud/api/v1/web/fcd05172-d405-4c41-b460-d62a1ff87d93/CCR/getCompanyCustomeView.json').then((response) => {
...
document.querySelector('button[role=submitQuery]').addEventListener('click', async function(event) {
    ...
    document.querySelector('button[role=submitQuery]').setAttribute('disabled', '')
    document.querySelectorAll('.loading-spin')[0].classList.remove('disabled')
    const date = new Date();
    ...
    if(content.message == "Successfully Raised Query") {
        ...
        document.querySelector('button[role=submitQuery]').removeAttribute('disabled')
