...
try {
    ...
    if(document.cookie === ''){location.href = "http://127.0.0.1:5000/tempLogin"}
    if(id.id == 0){
...
document.querySelectorAll('.ticket-box').forEach(val => {
    val.addEventListener('click', async () => {
        ...
        if(String(content.session_link) != "#")
            document.querySelector('a[target=_blank]').href = `http://127.0.0.1:5000/Chat?session=${content.session_link}`
